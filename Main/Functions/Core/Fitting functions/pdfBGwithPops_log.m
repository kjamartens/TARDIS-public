...
%---------------------------------------------------------
function [output, extraoutput] = pdfBGwithPops_log(inputdata,BGcurve_interp,output_BG_alldt,pdfSettings,callfromUI,varargin)
% keyboard
...
%% Extract loc unc if needed
vararginorig = varargin;
locuncorig = loc_unc;
if freefit_locunc
    var = cell2mat(varargin{1});
    loc_unc = var(end)/1e9;
    varargin{1} = mat2cell(var(1:end-1),[1],ones(1,size(var,2)-1));
else
%     keyboard
end
...
%     output_singlePop1 = pdfSinglePopFunction(inputdata(startpos(dt):endpos(dt)),D1,0,loc_unc,frame_time*dt);

    output_singlePop1 = pdfSinglePopFunction_log(inputdata(startpos(dt):endpos(dt)),D1,0,loc_unc,strobo_frame_time,frame_time*dt);
    %Correct input D for motion blurring (TESTING THIS)
%     keyboard
%     strobo_ratio = (1e-3/sqrt(frame_time*dt*6));%1e-3/(frame_time*dt);
%     output_singlePop1 = pdfSinglePopFunction_log(inputdata(startpos(dt):endpos(dt)),D1+strobo_ratio*D1,0,loc_unc,frame_time*dt);
    if populations > 1
...
%         output_singlePop2 = pdfSinglePopFunction(inputdata(startpos(dt):endpos(dt)),D2,0,loc_unc,frame_time*dt);
        output_singlePop2 = pdfSinglePopFunction_log(inputdata(startpos(dt):endpos(dt)),D2,0,loc_unc,strobo_frame_time,frame_time*dt);
    end
    if populations > 2
...
%         output_singlePop3 = pdfSinglePopFunction(inputdata(startpos(dt):endpos(dt)),D3,0,loc_unc,frame_time*dt);
        output_singlePop3 = pdfSinglePopFunction_log(inputdata(startpos(dt):endpos(dt)),D3,0,loc_unc,strobo_frame_time,frame_time*dt);
    end
...
%     normalize_area_log(xdata,input,functionp,DorBG,loc_unc,offset,norm_bins,frame_time)
%0.000000000050
%1e-13: +inf
%1e-12: 0
    [output_BG] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_BG,'pdfBGFunction',BGcurve_interp,loc_unc,1,10000,strobo_frame_time,frame_time*dt);
%     [output_singlePop1] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop1,'pdfSinglePopFunction',D1,loc_unc,1,BGcurve_interp,frame_time*dt);
    [output_singlePop1] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop1,'pdfSinglePopFunction',D1,loc_unc,1,10000,strobo_frame_time,frame_time*dt);
    
    if populations > 1
%         [output_singlePop2] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop2,'pdfSinglePopFunction',D2,loc_unc,f_pop_1,BGcurve_interp,frame_time*dt);

        [output_singlePop2] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop2,'pdfSinglePopFunction',D2,loc_unc,f_pop_1,10000,strobo_frame_time,frame_time*dt);
    end
    if populations > 2
        [output_singlePop3] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop3,'pdfSinglePopFunction',D3,loc_unc,f_pop_2,10000,strobo_frame_time,frame_time*dt);

%         [output_singlePop3] = normalize_area_log(inputdata(startpos(dt):endpos(dt)),output_singlePop3,'pdfSinglePopFunction',D3,loc_unc,f_pop_2,BGcurve_interp,frame_time*dt);
    end
...
%     figure(9);clf(9);
%     hold on
%     scatter(inputdata(1:size_dt(1)),output_singlePop1(:,1))
%     scatter(inputdata(1:size_dt(1)),output_BG(:,1))
%     scatter(inputdata(1:size_dt(1)),outputdt{1})
