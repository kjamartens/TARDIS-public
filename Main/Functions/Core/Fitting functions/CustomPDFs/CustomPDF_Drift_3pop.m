%% Custom PDF fitting routine - Three populations that are allowed to drift
...
%---------------------------------------------------------
function output = CustomPDF_Drift_3pop(xdata,parameters)
...
D_third = iterable_parameters(5)/1e12; %Diffusion coefficient of the second populatin
drift_third = iterable_parameters(6)/1e9*tau_value; %This is in nm 

%Fractions between the populations
fraction1 = iterable_parameters(7); %Fraction  (0-1)
fraction2 = iterable_parameters(8); %Fraction (0-1)
