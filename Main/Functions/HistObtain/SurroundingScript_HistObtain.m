load('C:\Users\Koen Martens\Documents\WUR info - data\Backup H Drive\MATLAB_BIPNAS\RelativeDistance_trackingAlternative\SimulatedTracks\OnePop_anaDDA\dens_1over2_kon_0_koff_0_Dfree_100.mat');
tic
frame_dist_BG = 9;
maxdist=2e-6;
frame_dist_Hist_arr=[1 2 3];%   4 5 6 7 8
vis = 1;
bgbinningnr = 100;
...
truthoffset{dt} = dt*1e-8*(baseline_to_be_added{dt}*bgbinningnr)/(size(reconstit_arr{dt},1)-(baseline_to_be_added{dt}*bgbinningnr))
end
...
custompdf = @(x,D)MLEfitter(x,D,truthoffset,loc_unc,frame_time,dtbinsize,norm_bins,ratio_baseline_to_data);
...
subplot(max(frame_dist_Hist_arr),3,1+3*(dt-1))
histogram(reconstit_arr{dt},linspace(0,maxdist,bgbinningnr+1));%,'Normalization','Probability');
hold on
xdata = [0:1e-8:2e-6];
plot(xdata,ones(size(xdata))*baseline_to_be_added{dt},'r-');
origdataoutput = output{dt}(:,2).*(size(reconstit_arr{dt},1)-baseline_to_be_added{dt}*(size(edges,2)-1));
origdataoutput = origdataoutput+baseline_to_be_added{dt};
plot(output{dt}(:,1),origdataoutput,'m-x')

subplot(max(frame_dist_Hist_arr),3,2+3*(dt-1))
origdataoutput = output{dt}(:,2);
plot(output{dt}(:,1),origdataoutput,'m-x')
hold on
...
ydata = output{dt}(:,1).*exp(-(output{dt}(:,1).^2)./(4*DMLEt*frame_time));
plot(output{dt}(:,1),ydata./sum(ydata),'b-','LineWidth',2,'DisplayName',['MLE: ' num2str(paramEsts*1e12)]);


subplot(max(frame_dist_Hist_arr),3,3+3*(dt-1))
histogram(reconstit_arr{dt},100,'Normalization','Probability');
hold on
xdata = [0:1e-8:maxdist];
loc_unc = 15e-9;
frame_time = 0.01*dt;
% flatline = (ratio_baseline_to_data.*size(reconstit_arr,1))./1e12;
flatline = ratio_baseline_to_data{dt};
...
ydata = xdata.*exp(-(xdata.^2)./(4*DMLE*frame_time))+truthoffset{dt};
...
%% function MLE
function [output] = MLEfitter(xdata,D,offset,loc_unc,frame_time,dtbinsize,norm_bins,ratio_baseline_to_data)
