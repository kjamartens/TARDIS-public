function [framescombined] = DDistributiongenerator_onlySingleStep_KM_URDA(koff,kon,Dfree,rangeD,locerrorpdfcorrected,maxindex,fx,fy,maxDindtracking,input,indexframetimerange)
...
if input.confinement == 1
    ...
    func = @(t1) Integralconfined(t1,Dfree,koff,kon,fx(:,1),fy(:,1),x,polyfunc); %NOTE: fx, fy lookup is altered from true anaDDA implementation
    %func = @(t1) Integralconfined(t1,Dfree,koff,kon,fx,fy,x,polyfunc);
    pdfarray = integral(func,0,1,'ArrayValued',true);
    %let's ignore the small values?
%     pdfarray(isnan(pdfarray)) = 0;
else
...
if input.confinement == 1
pdfarray(:,3) = pdfarray(:,3) + probmobile.*(1./sqrt(polyfunc(Dfree,fx(:,1)))).*(1./sqrt((polyfunc(Dfree,fy(:,1))))).*exp((-(1./polyfunc(Dfree,fx(:,1)))-(1./polyfunc(Dfree,fy(:,1)))).*rangeD(1:length(pdfarray))'/2).*besseli0_fast(((1./polyfunc(Dfree,fx(:,1)))-(1./polyfunc(Dfree,fy(:,1)))).*rangeD(1:length(pdfarray))'/2).*(rangeD(2)-rangeD(1));
