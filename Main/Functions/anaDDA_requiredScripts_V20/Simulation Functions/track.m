function tracks = track(positionList,maxdisp,param)
%;
...
timeList = positionList(:,dd);
shiftTime = circshift(timeList,1);
shiftTime = timeList(2:end) - shiftTime(2:end);
if  sum(shiftTime(find(shiftTime < 0))) ~= 0
...
% implanting unq directly
    indices = find(timeList ~= circshift(timeList,-1));
        count = length(indices);
        if count > 0
            result = indices;
        else  
            result = length(timeList) -1;
        end
 %%%%%%%%%%%%%%%%%%%%%%%       
        
result = [1,result',length(timeList)];
ngood = result(2) - result(1) + 1;
particleRows = 1:ngood;
pos = positionList(particleRows,1:dim+cellData);
istart = 2;
n = ngood;
...
for i=istart:z
    ...
    nPositions = result(i+1) - result(i);
    particleRows = 1:nPositions;
    particleRows = particleRows + result(i);
    
    if nPositions > 0
        
        positionsThisFrame = positionList(particleRows,1:dim+cellData);
        found = zeros(nPositions,1);
        ...
        if notnsqrd
            ...
            abi = fix(positionsThisFrame./blocksize);
            abpos = fix(pos./blocksize);
            si = zeros(nPositions,1);
            spos = zeros(n,1);
            dimm = zeros(dim,1);
            coff = 1.;
...
%                     end
                    ...
                    for d=1:dim     
                        distq = distq + (positionsThisFrame(map,d) - pos(j,d)).^2;
                    end
                    ltmax = distq < maxSquaredDisp;
            ...
            if ngood ~= 0 
                ...
                if ngood ~= 0 
                     %disp(size(w(ww)))
                    resx(ispan,w(ww)) = particleRows( which1(w(ww)));
                    found( which1( w(ww))) = 1;
                    rowtot( w(ww)) = 0;
            ...
            wh = find( pos(:,1) >= 0);
            nCurrentTrack = length(wh);
            if nCurrentTrack == 0 
                'There are no valid particles to track idiot!'
                break
            end
            xmat = zeros(nCurrentTrack,nPositions);
            count = 0;
            for kk=1:nCurrentTrack
                for ll=1:nPositions
                    xmat(kk,ll) = count;
                    count = count+1;
                end
            ...
            count = 0;
            for kk=1:nPositions
                for ll=1:nCurrentTrack
                    ymat(kk,ll) = count;
                    count = count+1;
                end
            end

            xmat = (mod(xmat,nPositions) + 1);
            ymat = (mod(ymat,nCurrentTrack) +1)';
...
%            disp(m)
            ...
            for d=1:dim
                thisFrame = positionsThisFrame(:,d);
                lastFrame = pos(wh,d);
                xm = thisFrame(xmat);
                ym = lastFrame(ymat(1:lenxn,1:lenxm));
                if size(xm) ~= size(ym)
                    xm = xm';
                end
                
                if d == 1
                    squaredDisplacement = (xm -ym).^2;
                    %dq = (x(xmat)-y(ymat(1:lenxn,1:lenxm))).^2;
                else
                    squaredDisplacement = squaredDisplacement + (xm-ym).^2;
                    %dq = dq + (x(xmat)-y(ymat(1:lenxn,1:lenxm)) ).^2;
                end
            end
            
            ltmax = (squaredDisplacement < maxSquaredDisp);
            
            % take cell numbers into account, if any
            if cellData>0
            thisFrame = positionsThisFrame(:,dim+cellData);
            lastFrame = pos(wh,dim+cellData);
            xm = thisFrame(xmat);
            ym = lastFrame(ymat(1:lenxn,1:lenxm));
            if size(xm) ~= size(ym)
                xm = xm';
                ...
                ltmax2 = (xm==ym);
                for logic = 1:numel(ltmax)
                    ltmax(logic) = ltmax(logic)&&ltmax2(logic);
                end
            end
        ...
        if nontrivial    
            ...
            for j=1:ydim
                ...
                for d=1:dim
                    %distq
                    distq = distq + (positionsThisFrame(labelx,d) - pos(labely(j),d)).^2;
                    %distq
                end
            if cellData>0
            thisFrame = positionsThisFrame(:,dim+cellData);
...
%                             keyboard
                ...
                if nnew > 1 
                    ...
                    sbr = circshift(sb,1);
                    sbl = circshift(sb,-1);
                    shiftTime(2:end) = find( sb(2:end) ~= sbr(2:end)) + 1;
...
%                end
                ...
                while checkflag ~= 2
                    ...
                    while who ~= -1
                        ...
                        if pt(who+1) ~= fi(who+1)
                            ...
                            if ngood > 0
                                ...
                                if who == nnew -1
                                    ww = find( lost == 0);
                                    dsq = sum(lensq(pt(ww))) + losttot*maxSquaredDisp;
                    ...
                    if checkflag == 1
                        plost = min([fix(mndisq/maxSquaredDisp) , (nnew -1)]);
                        if plost > nlost 
                            nlost = plost; 
                        else
                            checkflag = 2;
                ...
                resx(ispan,labely(bonds(minbonds,2))) = particleRows(labelx(bonds(minbonds,1)+1));
        ...
        if nww > 0 
            pos(w,:) = positionList( resx(ispan,w) , 1:dim+cellData);
            if goodenough > 0 
                nvalid(w) = nvalid(w) + 1;
            end
        ...
        if (nnew > 0) % & another keyword to workout inipos
            ...
            resx(ispan,n+1:end) = particleRows(newguys);
            pos = [[pos];[positionList(particleRows(newguys),1:dim+cellData)]];
            nmem = zeros(nnew,1);
            mem = [mem;nmem];
            nun = 1:nnew;
    ...
    if (ispan == zspan) | (i == z)
        ...
        if quiet~=1
            %%%modified 7-4-2016, SVDW, looks better when displayed
...
result = zeros(nolist,dd+1);
for j=1:dd
    result(:,j) = positionList(olist(:,1),j);
end
result(:,dd+1) = olist(:,2);

% this is uberize included for simplicity of a single monolithic code

ndat=length(result(1,:));
newtracks=result;
