function [Dfit,Dfitonestep,Dfittwostep,startD,tracks] = SimulationLocalizationandConfinement_GPU(input, plothist)
...
Dim_A1 = 1e-13; %has to be different value from Dim_A1, therefore very low
Dim_A2 = 1e-14; % has to be different value from Dim_A2, therefore very low
Dim_B1 = 1e-15;%has to be different value from Dim_B1, therefore very low
Dim_B2 = 1e-16;% has to be different value from Dim_B2, therefore very low
...
l = 1; % position index 
t = 0; %start time
cont = true(Nparticles,1,'gpuArray');
Poslist = zeros(Nparticles*(NFrames+1),3,'gpuArray');
startpos=zeros(Nparticles,3,'gpuArray');
sumcont = sum(cont(:));
%% Give each particle a starting position
while sumcont > 0
 startpos(cont,1)=xdimcell*rand(sumcont, 1,'single','gpuArray');
  startpos(cont,2:3,1) = (rand(sumcont,2,'single','gpuArray')-0.5)*(radiusofcell*2);
...
startD = rand(Nparticles,1,'gpuArray');
...
change = rand(Nparticles,1,'gpuArray');
...
k = sqrt(2*D*Steptime);
% initialise loop
running_particles = true(Nparticles,1,'gpuArray');
%% Set movement
...
dxyz = k.*(randn(sumrun,3,'gpuArray'));
newpos = startpos+dxyz;
else
dxyz = k(running_particles).*(randn(sumrun,3,'gpuArray'));
...
%now check all particles that are in the cell at the end of the step
particlesincell = newpos(:,1)>0 & (newpos(:,1)<xdimcell) & (squared<radiusofcell^2) & capcond;%.*running_particles;
particlesincell2 = false(Nparticles,1,'gpuArray');
...
%newpos = [];
newpos = newpos(~particlesincell,:)-2*dxyz(~particlesincell,:);
%Check if the molecules are inside the cell
squared = newpos(:,2).^2 + newpos(:,3).^2;
% capcond == 1 is necessary for continuation
%Splitting up in two parts 
...
%     startpos = newpos;
    t = t+Steptime;
    if  t > (runtime-0.5*Frametime) && (mod(t-runtime,Frametime) < 0.00000002 || mod(t,Frametime) > Frametime-0.00000002) 
